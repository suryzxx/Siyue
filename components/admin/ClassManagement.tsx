...
const ClassManagement: React.FC<ClassManagementProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    // Step 1: Basic Info
    contentMode: 'self' as 'self' | 'standard', // 自建课程
    courseId: '',
    name: '',
    campus: '',
    classroom: '',
  ...
  const generateSchedule = () => {
    ...
    // Simple mock generation: Just add days sequentially or based on frequency if selected
    // If frequency is empty, default to daily.
    ...
    while (count < lessonsCount) {
        ...
        newLessons.push({
            ...
            name: lessonName,
            date: dateStr,
            startTime: '14:00', // Default mock
            endTime: '16:00',
            status: 'pending',
            teacherId: formData.teacherId,
            pushTime: `${dateStr} 14:00`,
            pushStatus: 'pending'
        });
  ...
  }, [createStep, formData.frequency, formData.startDate, formData.courseId]);
  ...
  const handleCreateClass = () => {
    ...
    const newClass: ClassInfo = {
      id: randomId,
      name: formData.name,
      timeSlot: formData.displayTime || '每周固定时间',
      ...
      scheduleDescription: formData.displayTime,
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="bg-white rounded shadow-sm min-h-[400px]">
                      <div className="border-b border-gray-100 flex">
                          ...
                          <div className="px-6 py-4 text-blue-600 font-bold border-b-2 border-blue-600 cursor-pointer">学生</div>
                      </div>
                      
                      <div className="p-6">
                          <div className="flex items-center justify-between mb-4">
                              <div className="flex gap-4">
                                  <button className="px-4 py-1.5 border border-blue-500 text-blue-500 bg-blue-50 text-sm rounded">在班学生</button>
                              ...
                              <div className="flex gap-3">
                                  ...
                                  <button className="bg-blue-600 text-white px-4 py-1.5 rounded text-sm hover:bg-blue-700">添加学生</button>
                                  <button className="bg-blue-600 text-white px-4 py-1.5 rounded text-sm hover:bg-blue-700">批量导入学生</button>
                                  <button className="bg-blue-600 text-white px-4 py-1.5 rounded text-sm hover:bg-blue-700">整班续报</button>
                                  <button className="border border-blue-500 text-blue-500 px-4 py-1.5 rounded text-sm hover:bg-blue-50">导出</button>
                              </div>
                          </div>
                                      ...
                                      {displayStudents.length > 0 ? displayStudents.map(student => (
                                          <tr key={student.id} className="hover:bg-gray-50">
                                              ...
                                              <td className="p-4 text-center">
                                                  <button className="text-blue-500 hover:underline mx-1">转班</button>
                                                  <span className="text-gray-300">|</span>
                                                  <button className="text-blue-500 hover:underline mx-1">退班</button>
                                                  <span className="text-gray-300">|</span>
                                                  <button className="text-blue-500 hover:underline mx-1">停课</button>
                                              </td>
                                          </tr>
                                      )) : (
                                          <tr>
        ...
        <div className="border-t border-gray-100">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-[#F9FBFA] text-gray-600 font-medium border-b border-gray-200">
              <tr>
                <th className="p-4">班级ID</th>
                <th className="p-4">班级名称</th>
                <th className="p-4">绑定课程</th>
                <th className="p-4">课程类型</th>
                <th className="p-4">教学进度</th>
                <th className="p-4">已报人数</th>
                <th className="p-4">预招人数</th>
                <th className="p-4">年份</th>
                <th className="p-4">年级</th>
                <th className="p-4">学期</th>
                <th className="p-4">主教老师</th>
                <th className="p-4">校区</th>
                <th className="p-4">教室</th>
                <th className="p-4">助教</th>
                <th className="p-4">课程费用</th>
                <th className="p-4">班级状态</th>
                <th className="p-4">首课日期</th>
                <th className="p-4">上课时间</th>
              ...
              {filteredClasses.map(cls => {
                ...
                return (
                  <tr key={cls.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 text-gray-600">{progressText}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-gray-600">{cls.grade || '-'}</td>
                    ...
                    <td className="p-4 text-gray-600">{cls.classroom || '-'}</td>
                    <td className="p-4 text-gray-600">{assistant?.name || cls.assistant || '-'}</td>
                    <td className="p-4 text-gray-600">¥{cls.price || 0}</td>
                    <td className="p-4">{getStatusBadge(cls.status || 'pending')}</td>
                    <td className="p-4 text-gray-600">{cls.startDate}</td>
                    <td className="p-4 text-gray-600">{cls.timeSlot}</td>
            ...
            <div className="flex-1 px-32 pb-20">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-6 max-w-[800px]">
                            <div className="flex items-center">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4"><span className="text-red-500 mr-1">*</span>课程内容</label>
                                <div className="flex items-center gap-2">
                                    <input type="radio" checked={formData.contentMode === 'self'} readOnly className="text-blue-600" />
                                    <span className="text-sm text-gray-700">自建课程</span>
                                    <span className="text-xs text-gray-400 ml-2">自行准备内容，使用自建课程创建班级</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center">
                                ...
                                <div className="flex-1 flex gap-2">
                                    ...
                                    <button className="px-4 py-2 border border-blue-500 text-blue-500 rounded text-sm hover:bg-blue-50">选择课程</button>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center">
                                ...
                                <div className="flex-1 flex gap-2 items-center">
                                    ...
                                    <span className="text-blue-500 text-sm cursor-pointer whitespace-nowrap">查看教室课表</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4"><span className="text-red-500 mr-1">*</span>面授老师</label>
                                <div className="flex-1 flex gap-2 items-center">
                                    <select 
                                        ...
                                        className="flex-1 bg-white border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
                                    >
                                        <option value="">请选择面授老师</option>
                                        {TEACHERS.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                    </select>
                                    <span className="text-blue-500 text-sm cursor-pointer whitespace-nowrap">查看老师课表</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4"><span className="text-red-500 mr-1">*</span>是否允许冲突</label>
                                <div className="flex gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="conflict" checked={formData.allowConflict} onChange={() => setFormData({...formData, allowConflict: true})} />
                                        <span className="text-sm text-gray-700">是</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="conflict" checked={!formData.allowConflict} onChange={() => setFormData({...formData, allowConflict: false})} />
                            ...
                            <div className="flex items-center">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4">是否需要入学资格</label>
                                <div className="flex gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="qual" checked={formData.needQualification} onChange={() => setFormData({...formData, needQualification: true})} />
                                        <span className="text-sm text-gray-700">需要</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="qual" checked={!formData.needQualification} onChange={() => setFormData({...formData, needQualification: false})} />
                ...
                {createStep === 2 && (
                    <div className="space-y-6">
                        ...
                        <div className="text-xs text-red-500 mb-6 pl-4">
                            重要提醒：修改上课日期、频率、时间段都将重置课表的日期及时间，请慎重操作!!!
                        </div>

                        <div className="grid grid-cols-1 gap-6 max-w-[900px]">
                            <div className="flex items-center">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4"><span className="text-red-500 mr-1">*</span>开始上课日期</label>
                                <div className="flex-1 flex items-center gap-4">
                                    <input 
                                        type="date"
                                        value={formData.startDate}
                                        onChange={e => setFormData({...formData, startDate: e.target.value})}
                                        className="bg-gray-50 border border-gray-200 rounded px-3 py-2 text-sm w-64"
                                    />
                                    <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                        <input type="checkbox" checked={formData.skipHolidays} onChange={e => setFormData({...formData, skipHolidays: e.target.checked})} className="rounded text-blue-600" />
                                        跳过停课日
                                    </label>
                                    <span className="text-blue-500 text-sm cursor-pointer">设置停课日</span>
                                </div>
                            </div>

                            <div className="flex items-start">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4 pt-1"><span className="text-red-500 mr-1">*</span>频率</label>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {previewLessons.length > 0 ? previewLessons.map((l, idx) => (
                                            <tr key={l.id}>
                                                ...
                                                <td className="p-3 text-right text-gray-600">{l.date}</td>
                                            </tr>
                                        )) : (
                                            <tr>
                        ...
                        <div className="grid grid-cols-1 gap-6 max-w-[800px]">
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="flex items-center gap-6 text-sm text-gray-600">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" checked={formData.refundPolicy === 'unused'} onChange={() => setFormData({...formData, refundPolicy: 'unused'})} name="refund" />
                                            根据未上讲次退费
                                        </label>
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" checked={formData.refundPolicy === 'full'} onChange={() => setFormData({...formData, refundPolicy: 'full'})} name="refund" />
                                            前 1 讲退班，全额退费
                                            <span className="text-gray-300 text-xs">修改</span>
                                        </label>
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" checked={formData.refundPolicy === 'partial'} onChange={() => setFormData({...formData, refundPolicy: 'partial'})} name="refund" />
                                            后 1 讲退班，不退费用
                                            <span className="text-gray-300 text-xs">修改</span>
                                        </label>
                                    </div>
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="flex items-center gap-6 text-sm text-gray-600">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" checked={formData.materialRefundPolicy === 'no_return'} onChange={() => setFormData({...formData, materialRefundPolicy: 'no_return'})} name="materialRefund" />
                                            报名后不退
                                        </label>
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" checked={formData.materialRefundPolicy === 'return'} onChange={() => setFormData({...formData, materialRefundPolicy: 'return'})} name="materialRefund" />
                                            开课后不退
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center">
                                <label className="w-32 text-sm text-gray-500 text-right mr-4"><span className="text-red-500 mr-1">*</span>上课时间展示</label>
                                <input 
                                    value={formData.displayTime}
                                    onChange={e => setFormData({...formData, displayTime: e.target.value})}
                                    className="flex-1 border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500" 
                                    placeholder="请填写上课时间展示"
                                />
                            </div>
                        </div>
                    </div>
